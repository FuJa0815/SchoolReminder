    ...
    public static class WebUntisExtractor {
        public static async IAsyncEnumerable<Lesson> GetLessons(int classId = 192) {
            ...
            foreach (var p in json["data"]["result"]["data"]["elementPeriods"][classId.ToString()].ToList().AsParallel()) {
                ...
                var start = Task.Run(() => new DateTime(int.Parse(p["date"].ToString().Substring(0, 4)), int.Parse(p["date"].ToString().Substring(4, 2)), int.Parse(p["date"].ToString().Substring(6, 2)), int.Parse(p["startTime"].ToString().PadLeft(4, '0').Substring(0, 2)), int.Parse(p["startTime"].ToString().PadLeft(4, '0').Substring(2, 2)), 0));
                var end = Task.Run(() => new DateTime(int.Parse(p["date"].ToString().Substring(0, 4)), int.Parse(p["date"].ToString().Substring(4, 2)), int.Parse(p["date"].ToString().Substring(6, 2)), int.Parse(p["endTime"].ToString().PadLeft(4, '0').Substring(0, 2)), int.Parse(p["endTime"].ToString().PadLeft(4, '0').Substring(2, 2)), 0));
